class Menu {
  ...
  public void render() {
    ...
    fill(0);
    textSize(20);
    text(this.headingLine, this.pos.x+this.size.x/2-this.headingLine.length()/2, 15);
  ...
  public TimeLine getTimeLine(String name) {
    for(int i=0 ; i<this.parts.size() ; i++) {
      try {
        TimeLine ml = (TimeLine) this.parts.get(i);
...
abstract class MenuPart {
  ...
  public boolean mouseCollides() {
    return this.box.collidesWithMouse();
  }
...
public class TextBox extends MenuPart {
  ...
  public void backSpace() {
    ...
    for(int i=0 ; i<this.lines.size() ; i++) {
      ...
      if(ml.getContent()!="" && !allreadyBackSpaced) {
        ml.delete();
  ...
  public void render() {
    ...
    for(int i=0 ; i<this.lines.size() ; i++) {
      ...
      fill(0);
      text(ml.getContent(), super.pos.x-super.size.x/2, super.pos.y+(i+1)*15-super.size.y/2);
    }
  }
}
...
public class InputTextBox extends TextBox {
  ...
  private void write() {
    if(!this.mousePressedLastFrame && keyPressed && key!=CODED && key!=BACKSPACE) {
      super.write(key);
    }
    
    if(!this.mousePressedLastFrame && keyPressed && key==BACKSPACE) {
      super.backSpace();
    }
...
public class ScrollingMenu extends MenuPart {
  ...
  public MenuPart getCurPart() {
    return (MenuPart) this.parts.get(this.curPart);
  }
