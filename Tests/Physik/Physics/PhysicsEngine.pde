      ...
      for(int j = 0; j < bodies.size(); j++)
      {
        if(j != i)
        {
          ...
          if(coll != null)
          {
            ...
            bodies.get(i).velocity = v1.mult(m1).add(v1.mult(2).mult(m2).sub(v2.mult(m2))).div(m1 + m2);
            bodies.get(j).velocity = v2.mult(m2).add(v2.mult(2).mult(m1).sub(v1.mult(m1))).div(m1 + m2);
          }
        }
      }
    }
